...
class ForecastTemperatureDescription:
    """Description of a forecast temperature sensor."""

    key: str
    translation_key: str
    day_index: int  # 0 for today, 1 for tomorrow
    temp_type: str  # "max" or "min"
...
# Descriptions for forecast temperature sensors
...
async def async_setup_entry(
    ...
    for subentry_id, subentry in config_entry.subentries.items():
        ...
        # Add forecast temperature sensors
...
class NLForecastTemperatureSensor(
    ...
    """Generic sensor for forecast temperature (today/tomorrow, high/low)."""
    ...
    def __init__(
        ...
        self._temp_type = desc.temp_type

    @property
    def native_value(self):
        """Return the forecast temperature value."""
        try:
            ...
            _LOGGER.debug(
                "Error accessing forecast temperature for %s: %s",
                self.entity_description.key,
                err,
            )
            return None
